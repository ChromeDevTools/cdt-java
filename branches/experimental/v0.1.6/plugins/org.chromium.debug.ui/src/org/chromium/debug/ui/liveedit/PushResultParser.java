...
class PushResultParser {
  static LiveEditDiffViewer.Input createViewerInput(
      ...
      boolean previewMode) {
    Script script = filePair.getVmResource().getScript();
    IFile file = filePair.getFile();
    String newSourceTemp;
    try {
      byte[] newSourceBytes = ChromiumDebugPluginUtil.readFileContents(file);
      newSourceTemp = new String(newSourceBytes, "UTF-8");
    } catch (UnsupportedEncodingException e) {
      throw new RuntimeException(e);
    } catch (IOException e) {
      newSourceTemp = "Failed to read source, see Error Log";
      ...
      String getStatus(UpdatableScript.FunctionNode<?> rawFunction, NodeImpl nodeImpl) {
        ...
        if (asOldFunction == null) {
          ...
          switch (rawFunction.asOldFunction().getStatus()) {
            ...
            case NESTED_CHANGED: return "Function unchanged (only nested functions changed).";
            case CODE_PATCHED: return "Function will be updated.";
            case DAMAGED: {
              String message;
              if (nodeImpl.getPosition(Side.NEW) == null) {
                message = "Function is changed structurely.\n" +
                    "Already created instances of the function will stay intact.";
              } else {
                message =
                    "Function cannot be simply patched. It requires different context layout.\n" +
                    "Old instances will remain intact, new instances will have new body.\n" +
                    "Enclosing function will be changed to create new instances.";
              }
      ...
      String getStatus(UpdatableScript.FunctionNode<?> rawFunction, NodeImpl nodeImpl) {
        ...
        if (asOldFunction == null) {
          ...
          switch (asOldFunction.getStatus()) {
            case UNCHANGED: return "";
            case NESTED_CHANGED: return "Function unchanged (only nested functions changed).";
            case CODE_PATCHED: return "Function body is recompiled.";
            case DAMAGED: {
              String message;
              if (nodeImpl.getNewPositionInternal() == null) {
                message = "Function was changed structurely.\n" +
                    "Already created instances of the function are left intact " +
                    "and linked to old script version.";
              } else {
                message =
                  "Function wasn't simply patched. It now requires different context layout.\n" +
