...
public class V8ProtocolUtil {
   ...
   * Returns {@code null} if it is not, otherwise returns {@code script}. If
   * the script does not have a name (e.g. the former was generated by an
   * {@code eval()}) MODIFIES the original script by setting a hardcoded name.
   *
   ...
   *         otherwise
   */
  public static JSONObject scriptWithName(JSONObject script, JSONArray refs) {
    ...
    for (int i = 0, size = refs.size(); i < size; i++) {
      ...
      if (JsonUtil.getAsString(script, V8Protocol.BODY_NAME) == null) {
        script.put(V8Protocol.BODY_NAME.key, "(program)");
      }
      return script;
    }
