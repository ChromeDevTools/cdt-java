...
class PreviewLoader implements ValueSource<Optional<UpdatableScript.ChangeDescription>> {
    ...
    UpdatableScript.UpdateCallback callback = new UpdatableScript.UpdateCallback() {
      public void failure(String message) {
        Optional<UpdatableScript.ChangeDescription> error = createErrorOptional(
            new Message("Failed to get preview from VM: " + message, MessagePriority.WARNING));
        done(error);
      }
  ...
  // A consumer that receive the actual input parameter (V8FilePair).
  ...
  private static class Monitor {
    ...
    synchronized boolean updateResult(Optional<ChangeDescription> result, ScriptTargetMapping inputPair) {
      if (inputPair != input) {
        return false;
      }
      this.result = result;
  ...
  private static final Optional<UpdatableScript.ChangeDescription> EMPTY_DATA =
      createErrorOptional(new Message("Failed to load V8 preview data", MessagePriority.NONE));
}
