...
import org.chromium.debug.ui.DialogUtils.Getter;
...
class WizardLogicBuilder {
  ...
  WizardLogic create(final List<? extends ScriptTargetMapping> targetList) {
    ...
    // An derived value of selected VMs list; the list is non-empty or the value is error.
    final ValueProcessor<? extends Optional<List<ScriptTargetMapping>>> selectedVmValue =
        createProcessor(new Getter<Optional<List<ScriptTargetMapping>>>() {
    ...
    Getter<? extends Optional<Boolean>> singleVmSelectedExpression = handleErrors(
    ...
    // A simple value converter that wrap wizard delegate as UI-aware wizard finisher.
   ...
   * return additional warnings messages.
   */
  ...
  private PreviewAndOptionPath createSingleVmPath(PageImpl<?> basePage,
    ...
    final ValueProcessor<ScriptTargetMapping> singleVmValue = createProcessor(new Getter<ScriptTargetMapping>() {
    ...
    ValueConsumer textualDiffInputSetter = new ValueConsumer() {
      public void update(Updater updater) {
        ...
        CompareChangesAction.LiveEditCompareInput input =
            new CompareChangesAction.LiveEditCompareInput(filePair.getFile(), filePair.getVmResource());
    ...
    // A complex asynchronous value source that feeds in an update preview data from V8.
    ...
    // Parses raw preview value and convert it in form suitable for the viewer; also handles
    // errors that becomes warnings.
    final ValueProcessor<Optional<LiveEditDiffViewer.Input>> previewValue =
        createProcessor(handleErrorsAddNew(new NormalExpression<Optional<LiveEditDiffViewer.Input>>() {
    ...
    final ValueProcessor<Optional<Void>> warningValue = createProcessor(new Getter<Optional<Void>>() {
    ...
    final ValueProcessor<? extends Optional<FinisherDelegate>> wizardFinisher =
        createProcessor((
            new Getter<Optional<FinisherDelegate>>() {
